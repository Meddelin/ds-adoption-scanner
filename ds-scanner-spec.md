...
40. `scanner/ds-prescan.ts` (новый) — `preScanDesignSystems()`, `buildFamilyCatalog()`, `buildFamilyLookup()`, `groupIntoFamilies()`; переиспользует `parseFileExports()` из library-prescan; только PascalCase value-exports (interfaces/types исключены). Алгоритм `getFamilyName`: берём путь от dsRoot до файла, пропускаем ведущие GENERIC_DIRS, первый оставшийся сегмент = семья. Это корректно обрабатывает любую глубину вложенности (`EmptyState/EmptyStateButton/Btn.tsx` → семья `EmptyState`).
...
42. `scanner/library-prescan.ts` — расширить `LibraryComponentEntry` полем `dsFamily?`; `preScanLibraries()` принимает `DSCatalog` и заполняет `dsFamily` для DS-backed компонентов. Реализация: `ExportInfo.dsImportedNames[]` собирает конкретные DS-компоненты, импортированные в файл; `buildComponentMap` резолвит семью через `dsFamilyLookup.get(dsImportedName)` (не через имя самого компонента).
