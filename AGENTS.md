...
## Codex Persistent Bootstrap

Use these project-local files so each new task does not start from zero:

- `.codex/PROJECT_CONTEXT.md` — stable architecture + rules + done checklist
- `.codex/TASK_PLAYBOOK.md` — task flow and change-type test matrix
- `.codex/SESSION_MEMORY.md` — rolling memory of recent baseline/findings
- `scripts/codex-bootstrap.mjs` — quick project snapshot (`npm run codex:bootstrap`)

Recommended start sequence for every task:
1. Run `npm run codex:bootstrap`
2. Read `.codex/PROJECT_CONTEXT.md`
3. Execute from `.codex/TASK_PLAYBOOK.md`
4. Append concise notes to `.codex/SESSION_MEMORY.md` after finishing
